<template>
  <div>
    ...
    <v-layout row wrap>
      <v-flex>
        <v-text-field v-model="pokemon.id" label="ID" />
      </v-flex>
      <v-flex>
      ...
      <v-flex xs3>
        <v-text-field v-model="pokemon.generation" label="Generation" />
      </v-flex>
      <v-flex xs3>
      ...
      <v-btn class="bg-primary white--text" v-on:click="add">Add</v-btn>
    </v-layout>
  </div>
</template>
...
export default {
  ...
  methods: {
    async add() {
      await pokemonApi.add(this.pokemon)
